...
export const AdminDashboard = () => {
    ...
    const handleDeleteClick = async (profileId: string) => {
        ...
        try {
            ...
            alert('Usuário excluído com sucesso (Auth + Perfil).');
        } catch (err: any) {
            console.error(err);
    ...
    const handleAddAllowedEmail = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert('Parceiro autorizado com sucesso!\n\nLink de Convite Personalizado:\n' + link);
        } catch (err: any) {
    ...
    const handleRemoveAllowedEmail = async (email: string) => {
        if (!confirm(`Remover acesso de ${email}? Ele não poderá criar novas contas.`)) return;
        try {
    ...
    const handleManualRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(err);
            alert('Erro no cadastro Manual: ' + err.message);
        } finally {
            setRegLoading(false);
        }
    };
                                        ...
                                        <td className="px-6 py-4 text-right flex justify-end gap-2">
                                            <button
                                                onClick={() => {
                                                    ...
                                                    navigator.clipboard.writeText(url);
                                                    alert('Link de convite personalizado copiado!\n\n' + url);
                                                }}
