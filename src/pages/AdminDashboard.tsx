...
import { format, parseISO, isAfter, addDays, addMonths, addYears } from 'date-fns';
import { Shield, Search, Calendar, CheckCircle, XCircle, AlertTriangle, Edit, Save, X, Loader2 } from 'lucide-react';
...
export const AdminDashboard = () => {
    ...
    const handleEditClick = (profile: Profile) => {
        ...
        // Format for HTML input date (YYYY-MM-DD), but valid_until includes time, so we strip it
    ...
    const handleSaveDate = async (profileId: string) => {
        ...
        try {
            ...
            const { error } = await supabase
                .from('profiles')
                .update({ valid_until: isoDate })
                .eq('id', profileId);

            if (error) throw error;

            // Update local state
            setProfiles(profiles.map(p => p.id === profileId ? { ...p, valid_until: isoDate } : p));
            setEditingId(null);
        } catch (err: any) {
            console.error(err);
    ...
    const getStatus = (validUntil: string) => {
        if (!validUntil) return <span className="text-slate-400">Indefinido</span>;
