...
export const Equipe = () => {
    ...
    const handleForceAccept = async (inviteId: string) => {
        try {
            ...
            // Reload to refresh lists (Member might appear if they were already linked but invite was stuck)
            // If they are not linked yet, they just vanish from "Pending", which is also cleaner.
            loadData();
            alert('Convite marcado como aceito.');
        } catch (error: any) {
            alert('Erro: ' + error.message);
        }
    };
    ...
    const removeMember = async (memberId: string) => {
        if (!confirm('Remover membro da equipe? O usuário perderá o vínculo com a clínica.')) return;

        const { error } = await supabase
            .from('profiles')
            .update({ owner_id: null }) // Unlink
            .eq('id', memberId);
    ...
    const copyLink = (token: string) => {
        const link = `${window.location.origin}/entrar/${token}`; // Route: /entrar/:token
